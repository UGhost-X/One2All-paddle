...
from utils.patchcore_trainer import trainer
...
def train_category(category_name, dataset_dir):
    ...
    while True:
        ...
        if logs:
            for log in logs[-3:]:  # 只打印最后3条
                if not hasattr(train_category, '_last_log_idx') or logs.index(log) >= train_category._last_log_idx:
                    logger.info(f"  {log}")
            train_category._last_log_idx = len(logs)
