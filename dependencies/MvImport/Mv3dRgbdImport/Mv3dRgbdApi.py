...
class Mv3dRgbd():
    ...
    # remark 设置触发模式为打开，设置触发源为软触发并执行。对于支持触发id功能的设备，软触发得到的图像帧包含触发id
    ...
    #       深度图格式:C16
    #       RGB图格式:RGB8_Planar/YUV422/YUV420SP_NV12/YUV420SP_NV21/Jpeg
    ...
    #       rgb image format: RGB8_Planar/YUV422/YUV420SP_NV12/YUV420SP_NV21/Jpeg
    ...
    # *******************************************************************************************************
    #brief  保存图像帧数据，仅支持Hibag格式
    #param  handle                            [IN]            相机句柄
    #param  pstFrame                          [IN]            图像帧数据
    #param  enFileType                        [IN]            文件类型
    #param  chFileName                        [IN]            文件名称
    #return 成功，返回MV3D_RGBD_OK；错误，返回错误码 

    #brief  save frame data to file, only support hibag file type
    #param  handle                            [IN]            camera handle
    #param  pstFrame                          [IN]            frame data 
    #param  enFileType                        [IN]            file type
    #param  chFileName                        [IN]            file name
    #return Success, return MV3D_RGBD_OK. Failure, return error code
    def MV3D_RGBD_SaveImageEx(self, pstFrame, enFileType, chFileName):
        Mv3dRgbdDll.MV3D_RGBD_SaveImageEx.argtype = (c_void_p, c_void_p, c_int, c_void_p)
        Mv3dRgbdDll.MV3D_RGBD_SaveImageEx.restype = c_uint
        # C：MV3D_RGBD_API MV3D_RGBD_STATUS MV3D_RGBD_SaveImageEx(void* handle, MV3D_RGBD_FRAME_DATA* pstFrame, Mv3dRgbdFileType enFileType, const char* chFileName);
        return Mv3dRgbdDll.MV3D_RGBD_SaveImageEx(self.handle, pstFrame, enFileType, chFileName.encode('ascii'))
    # *******************************************************************************************************    
    ...
    # *******************************************************************************************************
    #brief  纹理点云存图接口
    #       纹理图格式: RGB8_Planar/YUV422/YUV420SP_NV12/YUV420SP_NV21/Jpeg
    ...
    #       texture image format:RGB8_Planar/YUV422/YUV420SP_NV12/YUV420SP_NV21/Jpeg
    ...
    #  brief  获取实例分割信息
    #  param  handle                      [IN]            相机句柄
    #  param  pstInstanceSegInfo          [IN][OUT]       数据指针
    #  return 成功，返回MV3D_RGBD_OK；错误，返回错误码

    #  brief  get instanceSeg info
    #  param  handle                      [IN]            camera handle
    #  param  pstInstanceSegInfo          [IN][OUT]       data set pointer
    #  return Success, return MV3D_RGBD_OK. Failure, return error code
    def MV3D_RGBD_GetInstanceSegInfo(self, pstInstanceSegInfo):
        Mv3dRgbdDll.MV3D_RGBD_GetInstanceSegInfo.argtype = (c_void_p, c_void_p)
        Mv3dRgbdDll.MV3D_RGBD_GetInstanceSegInfo.restype = c_uint
        # C：MV3D_RGBD_API MV3D_RGBD_STATUS MV3D_RGBD_GetInstanceSegInfo(HANDLE handle, MV3D_RGBD_INSTANCE_SEG_INFO* pstInstanceSegInfo);
        return Mv3dRgbdDll.MV3D_RGBD_GetInstanceSegInfo(self.handle, pstInstanceSegInfo)
    # *******************************************************************************************************

    # ***********************************************************************
    #  brief  获取单件分离信息
    #  param  handle                      [IN]            相机句柄
    #  param  pstSingulatePkgInfo         [IN][OUT]       数据指针
    #  return 成功，返回MV3D_RGBD_OK；错误，返回错误码

    #  brief  get singulate package info
    #  param  handle                      [IN]            camera handle
    #  param  pstSingulatePkgInfo         [IN][OUT]       data set pointer
    #  return Success, return MV3D_RGBD_OK. Failure, return error code
    def MV3D_RGBD_GetSingulatePkgInfo(self, pstSingulatePkgInfo):
        Mv3dRgbdDll.MV3D_RGBD_GetSingulatePkgInfo.argtype = (c_void_p, c_void_p)
        Mv3dRgbdDll.MV3D_RGBD_GetSingulatePkgInfo.restype = c_uint
        # C：MV3D_RGBD_API MV3D_RGBD_STATUS MV3D_RGBD_GetSingulatePkgInfo(HANDLE handle, MV3D_RGBD_SINGULATE_PKG_INFO* pstSingulatePkgInfo);
        return Mv3dRgbdDll.MV3D_RGBD_GetSingulatePkgInfo(self.handle, pstSingulatePkgInfo)
    # *******************************************************************************************************

    # ***********************************************************************
    #  brief  获取抓取信息
    #  param  handle                      [IN]            相机句柄
    #  param  pstGraspInfo                [IN][OUT]       数据指针
    #  return 成功，返回MV3D_RGBD_OK；错误，返回错误码

    #  brief  get grasp info
    #  param  handle                      [IN]            camera handle
    #  param  pstGraspInfo                [IN][OUT]       data set pointer
    #  return Success, return MV3D_RGBD_OK. Failure, return error code
    def MV3D_RGBD_GetGraspInfo(self, pstGraspInfo):
        Mv3dRgbdDll.MV3D_RGBD_GetGraspInfo.argtype = (c_void_p, c_void_p)
        Mv3dRgbdDll.MV3D_RGBD_GetGraspInfo.restype = c_uint
        # C：MV3D_RGBD_API MV3D_RGBD_STATUS MV3D_RGBD_GetGraspInfo(HANDLE handle, MV3D_RGBD_GRASP_INFO* pstGraspInfo);
        return Mv3dRgbdDll.MV3D_RGBD_GetGraspInfo(self.handle, pstGraspInfo)
    # *******************************************************************************************************
    ...
    #  param  pUser                       [IN]            User defined variable
    #  return Success, return MV3D_RGBD_OK. Failure, return error code
    def MV3D_RGBD_RegisterIMUDataCallBack(self, cbImuOutput, pUser):
        Mv3dRgbdDll.MV3D_RGBD_RegisterIMUDataCallBack.argtype = (c_void_p, c_void_p, c_void_p)
        Mv3dRgbdDll.MV3D_RGBD_RegisterIMUDataCallBack.restype = c_uint
        # C：MV3D_RGBD_API MV3D_RGBD_STATUS MV3D_RGBD_RegisterIMUDataCallBack(HANDLE handle, MV3D_RGBD_IMUDataCallBack cbOutput, void* pUser);
        return Mv3dRgbdDll.MV3D_RGBD_RegisterIMUDataCallBack(self.handle, cbImuOutput, pUser)
    # *******************************************************************************************************
